...
async function BrokerListings() {
  const { session } = await requireBroker();
  const listings = await listBrokerProperties(session.user.id);

  if (listings.length === 0) {
    return (
      <Card className="rounded-3xl">
        <CardContent className="space-y-4 p-8 text-center">
          <p className="text-sm text-muted-foreground">No listings in your book yet.</p>
          ...
          <CardContent className="flex flex-wrap items-center justify-between gap-4 p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {row.property.city} · {formatPrice(row.property.price)} · {row.bookings} viewings
              </p>
            </div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Properties you represent between buyers and sellers.
          </p>
        </div>
        <Button asChild className="rounded-full">
          <Link href="/broker/properties/new">Add listing</Link>
